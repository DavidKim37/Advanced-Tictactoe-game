...
def check(board, boards, currentblock, dead, color, player):
    filled = []
    gameend = False
    algorithm = [["a1", "b1", "c1"], ["a2", "b2", "c2"], ["a3", "b3", "c3"], ["a1", "a2", "a3"], ["b1", "b2", "b3"],
                 ["c1", "c2", "c3"], ["a1", "b2", "c3"], ["a3", "b2", "c1"]]
...
def checkmate(color, dead, board):
    algorithm = [["A", "B", "C"], ["D", "E", "F"], ["G", "H", "I"], ["A", "D", "G"], ["B", "E", "H"], ["C", "F", "I"],
                 ["A", "E", "I"], ["G", "E", "C"]]
    filled = []
    for i in dead:
        if board[i + "a1"] == color:
...
def movepiece(currentblock, dead, board, player, dictio, boards, color):
    ...
    try:
        ...
        if randomized:
            print(
                "You entered an invalid response. Sad face. Therefore, a random block was selected for you. More sad "
                "face.")
            randomized = False
        print(currentblock)
            ...
            if usermove[1] == "1" or usermove[1] == "2" or usermove[1] == "3":
                if board[currentblock + usermove][0] == " ":
                    if player == "X":
                        ...
                        gameend, dead, board = check(board, boards, currentblock, dead, color + "   " + "\u001B[0m",
                                                     player)
                        player = "O"
                        color = "\u001B[44m"
                    else:
    ...
    try:
        ...
        movepiece(currentblock, dead, board, player, dictio, board, color)
